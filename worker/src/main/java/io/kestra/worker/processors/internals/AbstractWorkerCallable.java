...
public abstract class AbstractWorkerCallable implements Callable<State.Type> {
    ...
    /**
     * Interrupts the running job. If {@code state} is non-null, the job is marked to eventually
     * report that state as its outcome instead of the one {@link #doCall()} would otherwise produce
     * (e.g. {@code KILLED} for a real kill, {@code CANCELLED} for a fail-fast interrupt). Passing
     * {@code null} interrupts without changing the reported outcome — used by the timeout path,
     * which still wants {@code FAILED} afterward.
     */

...
public class ExecutionKilledManager {
    ...
    public void onKillReceived(ExecutionKilled killed) {
        if (killed instanceof ExecutionKilledExecution killedExecution) {
            ...
            // Write-ahead: remember every target as pending BEFORE scanning currently-running jobs.
            // A job racing to register() concurrently always does "put into runningJobs, then check
            // pending" (see register() below) — pairing that with "check running, then write pending"
            // here would leave a window where a job registering exactly in between is missed by both
            // checks. With the pending write happening first, at least one of the two checks (this
            // scan, or register()'s pending lookup) is guaranteed to observe the job.
            killedTaskRuns.getTaskRunIds().forEach(taskRunId -> pendingTaskRunInterrupts.put(taskRunId, killedTaskRuns.getTaskRunState()));

            runningJobs.forEach((_, killableJob) ->
            {
                if (killableJob.job() instanceof WorkerTask workerTask && killedTaskRuns.isEqual(workerTask)) {

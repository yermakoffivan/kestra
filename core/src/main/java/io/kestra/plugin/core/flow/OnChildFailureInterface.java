 ...
 * Implemented by flowables that run their children truly concurrently ({@link Parallel}, {@link Dag}) —
 * meaningless for a flowable that only ever runs one child at a time.
 */
public interface OnChildFailureInterface {
     ...
     * the scope of task runs to interrupt when a child fails.
     */
